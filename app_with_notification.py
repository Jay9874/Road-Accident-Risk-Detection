...
import time  # For time-based logic
...
SHAPE_PREDICTOR_PATH = (
    "shape_predictor_68_face_landmarks.dat"  # Download and place this file!
)
...
# --- Simulated GPS Coordinates (for demonstration purposes only) ---
# In a real application, you'd integrate with actual GPS hardware or location services.
SIMULATED_LATITUDE = 34.0522  # Example: Los Angeles latitude
SIMULATED_LONGITUDE = -118.2437  # Example: Los Angeles longitude
...
if not os.path.exists(SHAPE_PREDICTOR_PATH):
    print(
        f"Error: Dlib shape predictor not found at {SHAPE_PREDICTOR_PATH}. Please download and place it as instructed."
    )
    exit()
...
try:
    ...
    print("Please check your Twilio Account SID and Auth Token.")
    twilio_client = (
        None  # Set to None to prevent further errors if initialization fails
    )
...
if not cap.isOpened():
    print(
        "Error: Could not open webcam. Make sure your camera is connected and not in use by another application. Try changing `cv2.VideoCapture(0)` to `cv2.VideoCapture(1)` or higher if you have multiple cameras."
    )
    exit()

# Get camera FPS and calculate drowsiness frame threshold
...
if camera_fps > 0:
    ...
    print(
        "Warning: Could not determine camera FPS. Defaulting to DROWSINESS_FRAME_THRESHOLD = 30 frames."
    )
    DROWSINESS_FRAME_THRESHOLD = 30  # Fallback value
...
while True:
    ...
    # Flags for overall frame status (reset for each frame)
    ...
    for rect in rects:
        ...
        if my_start < my_end and mx_start < mx_end:  # Ensure valid crop
            ...
            if (
                mouth_img_gray_cropped.shape[0] > 0
                and mouth_img_gray_cropped.shape[1] > 0
            ):
                input_yawn_model = cv2.resize(
                    mouth_img_gray_cropped, (YAWN_IMG_WIDTH, YAWN_IMG_HEIGHT)
                )
                # Display resized mouth for debugging, but only if it's a valid image
                if input_yawn_model.size > 0:
                    cv2.imshow("Cropped Mouth for Yawn CNN", input_yawn_model)
                ...
                if predicted_yawn_label == "Yawn":
                    ...
                    cv2.rectangle(
                        frame, (mx_start, my_start), (mx_end, my_end), (0, 0, 255), 2
                    )  # Red for Yawn
                    cv2.putText(
                        frame,
                        f"Yawn: {yawn_confidence:.1f}%",
                        (mx_start, my_start - 10),
                    ...
                    cv2.rectangle(
                        frame, (mx_start, my_start), (mx_end, my_end), (0, 255, 0), 1
                    )  # Green for No Yawn
                    cv2.putText(
                        frame,
                        f"No Yawn: {yawn_confidence:.1f}%",
                        (mx_start, my_start - 10),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 255, 0),
                ...
                print(
                    f"DEBUG: Yawn Prediction: {prediction_yawn[0]}, Label: {predicted_yawn_label}, Conf: {yawn_confidence:.2f}%"
                )
            else:
                print(
                    "DEBUG: Mouth crop resulted in empty image. Skipping yawn prediction."
                )
        else:
            print(
                f"DEBUG: Mouth invalid crop coordinates: ({mx_start},{my_start}) to ({mx_end},{my_end}). Skipping mouth processing."
            )
        ...
        for i, eye_points in enumerate([left_eye_points, right_eye_points]):
            ...
            if ey_start < ey_end and ex_start < ex_end:  # Ensure valid crop
                ...
                if (
                    eye_img_gray_cropped.shape[0] > 0
                    and eye_img_gray_cropped.shape[1] > 0
                ):
                    input_eye_model = cv2.resize(
                        eye_img_gray_cropped, (EYE_IMG_WIDTH, EYE_IMG_HEIGHT)
                    )
                    # Display resized eye for debugging, but only if it's a valid image
                    if input_eye_model.size > 0:
                        cv2.imshow(f"Cropped Eye {i+1} for Eye CNN", input_eye_model)
                    ...
                    if predicted_eye_label == "closed_eye":
                        eyes_closed_this_frame = True  # Set flag if any eye is closed
                        color = (0, 0, 255)  # Red for closed

                    cv2.rectangle(
                        frame, (ex_start, ey_start), (ex_end, ey_end), color, 1
                    )
                    cv2.putText(
                        frame,
                        f"{predicted_eye_label.replace('_', ' ').capitalize()}: {eye_confidence:.1f}%",
                        (ex_start, ey_start - 10),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.4,
                        color,
                        1,
                    ...
                    print(
                        f"DEBUG: Eye {i+1} Prediction: {prediction_eye[0]}, Label: {predicted_eye_label}, Conf: {eye_confidence:.2f}%"
                    )
                else:
                    print(
                        f"DEBUG: Eye {i+1} crop resulted in empty image. Skipping eye prediction."
                    )
            else:
                print(
                    f"DEBUG: Eye {i+1} invalid crop coordinates: ({ex_start},{ey_start}) to ({ex_end},{ey_end}). Skipping eye processing."
                )

    # --- Overall Drowsiness Status & Alert Logic ---
    overall_drowsiness_status = "Alert"
    status_color = (0, 255, 0)  # Green for alert

    if len(rects) > 0:  # Only assess drowsiness if a face is detected
        ...
        if (
            ...
            if send_drowsiness_alert(SIMULATED_LATITUDE, SIMULATED_LONGITUDE):
                ...
                time.sleep(2)
                sys.exit("One alert sent, stopping the car.")
            print("----------------------------------------------------------------")
        ...
        drowsy_frame_count = 0  # Reset counter if no face
