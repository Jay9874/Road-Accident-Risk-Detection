...
RECIPIENT_PHONE_NUMBER = os.getenv("RECIPIENT_PHONE_NUMBER")              # The phone number to send the alert to (must be verified in Twilio trial)

# --- Drowsiness Alert Logic ---
DROWSINESS_FRAME_THRESHOLD = 30 # Number of consecutive frames indicating drowsiness to trigger alert
...
try:
    ...
    twilio_client = None # Set to None to prevent further errors if initialization fails
...
def send_drowsiness_alert(latitude, longitude):
    ...
    try:
        ...
        print("Please check your Twilio phone number, recipient number (must be verified for trial accounts), and account balance.")
        return False
...
while True:
    ...
    for rect in rects:
        ...
        if my_start < my_end and mx_start < mx_end: # Ensure valid crop
            ...
            if mouth_img_gray_cropped.shape[0] > 0 and mouth_img_gray_cropped.shape[1] > 0:
                ...
                print("DEBUG: Mouth crop resulted in empty image. Skipping yawn prediction.")
        else:
            print(f"DEBUG: Mouth invalid crop coordinates: ({mx_start},{my_start}) to ({mx_end},{my_end}). Skipping mouth processing.")
        ...
        for i, eye_points in enumerate([left_eye_points, right_eye_points]):
            ...
            if ey_start < ey_end and ex_start < ex_end: # Ensure valid crop
                ...
                if eye_img_gray_cropped.shape[0] > 0 and eye_img_gray_cropped.shape[1] > 0:
                    ...
                    print(f"DEBUG: Eye {i+1} crop resulted in empty image. Skipping eye prediction.")
            else:
    ...
    if len(rects) > 0: # Only assess drowsiness if a face is detected
        ...
        if drowsy_frame_count >= DROWSINESS_FRAME_THRESHOLD and not alert_sent:
            ...
            if send_drowsiness_alert(SIMULATED_LATITUDE, SIMULATED_LONGITUDE):
                ...
                sys.exit("Stopping because an alert was sent.")
            print("----------------------------------------------------------------")
